...
recon = normalize(patch2im(Y(:,1:size(Y,2)/3), patchsize));
...
Alpha1 = struct('d', 3, 's', 3, 'n', 1e-3, 'pi', 2);
Beta1 = struct('d', 0.3, 's', 0.3, 'n', 1e-3, 'pi', 10);

Alpha2 = struct('d', 1e-1, 's', 1e-1, 'n', 1e-3, 'pi', 1);
Beta2 = struct('d', 1e-1, 's', 1e-1, 'n', 1e-3, 'pi', 5);
...
%     [ muD2, muS2, PI2, Gamma2, Palpha2, Pbeta2 ] = VB_Update( Y2, muD2, muS2, PI2, Gamma2, Palpha2, Pbeta2, Alpha2, Beta2, c2 );
    ...
    % Test here only
%     Y_approx = muD1*(muS1.*binornd(ones(K1,N), PI1));
